...
最初产品盘点针对 Agent Skill 与本地辅助脚本。此后 `ai_life_apply` 根目录已增加 Vue/FastAPI 演示骨架、SQLite 项目/任务记录和标题卡渲染；正式多用户认证、真实媒体生成和完整视频合成仍待开发。下面保留原制作能力的复用分析；当前实现审计见 [technical-design.md](./technical-design.md)，验证状态见 [development-status.md](./development-status.md)。
...
详细技术方案、数据模型、API、工作流、验收与 AI 开发任务已整理在 [technical-design.md](./technical-design.md) 及其配套文档中。后续按该基线完成页面交互和可运行功能切片；产品范围变化需同步更新对应契约。
